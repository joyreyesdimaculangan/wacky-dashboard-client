    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="relative bg-white p-8 rounded-lg shadow-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <h2 class="text-2xl font-bold mb-6 text-green-700">Add Content</h2>
          <form [formGroup]="addPackagesForm" class="p-4 md:p-5" (submit)="submitForm()">
            <div class="grid gap-4 mb-4 grid-cols-2">
              ...
              <div formArrayName="additionalInclusions">
                  ...
                  <div *ngFor="let inclusion of additionalInclusions.controls; let i = index">
                    <div [formGroupName]="i" class="mb-4">
                      <input formControlName="name" placeholder="Inclusion Name" class="form-input" />
                      <button type="button" (click)="removeInclusion(i)" class="text-red-500">Remove</button>
                    </div>
                  </div>
                  <button type="button" (click)="addInclusion()" class="text-green-500">Add Inclusion</button>
                </div>
                ...
                <div formArrayName="addOns">
                  ...
                  <div *ngFor="let addOn of addOns.controls; let i = index">
                    <div [formGroupName]="i" class="mb-4">
                      <input formControlName="name" placeholder="Add-On Name" class="form-input" />
                      <button type="button" (click)="removeAddOn(i)" class="text-red-500">Remove</button>
                    </div>
                  </div>
                  <button type="button" (click)="addAddOn()" class="text-green-500">Add Add-On</button>
                </div>
...
export class PackagesCrmComponent { 
  ...
  addInclusion() {
    this.additionalInclusions.push(this.fb.group({
      name: ['', Validators.required],
    }));
  }
  ...
  // loadPackageData(packageID: string) {
  //   this.packageService.getPackageById(packageID).subscribe(packageData => {
  ...
  //     this.loadAdditionalInclusions(packageID);
  //     this.loadAddOns(packageID);
  //   });
  // }

  // loadAdditionalInclusions(packageID: string) {
  //   this.additionalInclusionsService.getInclusionById(packageID).subscribe(inclusions => {
  //     this.additionalInclusions.clear();
  //     inclusions.forEach((inclusion: { name: any; }) => {
  //       this.additionalInclusions.push(this.fb.group({
  //         name: [inclusion.name, Validators.required],
  //       }));
  //     });
  //   });
  // }

  // loadAddOns(packageID: string) {
  //   this.addOnService.getAddOnById(packageID).subscribe(addOns => {
  //     this.addOns.clear();
  //     addOns.forEach((addOn: { name: any; }) => {
  //       this.addOns.push(this.fb.group({
  //         name: [addOn.name],
  //       }));
  //     });
    ...
    this.addPackagesSubscription.add(
      this.packageService.createPackage(packageData).subscribe({
        next: (createdPackage: Packages) => {
          ...
          const additionalInclusions$ = packageData.additionalInclusions.map((inclusion: PackageInclusions) =>
            this.additionalInclusionsService.createInclusion({ ...inclusion, packageID }).pipe(
              catchError(error => {
          ...
          forkJoin([...additionalInclusions$, ...addOns$]).subscribe({
            next: (results) => {
              ...
              const validInclusions = results.slice(0, packageData.additionalInclusions.length).filter(result => result !== null);
              const validAddOns = results.slice(packageData.additionalInclusions.length).filter(result => result !== null);
